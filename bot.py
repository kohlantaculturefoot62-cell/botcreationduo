...
# ==========================================
# 0. GESTION AUTOMATIQUE DES POLICES (PILLOW)
# ==========================================
...
CATEGORIES_CIBLES = [
    "confessional", "confessionnal", "camps", "camp", "duo jaune", "duo rouge",
    "trio", "quatuor", "equipe rouge", "equipe jaune", "destin lies", "destins lies",
    "destin lie", "Alliés de l’ombre"
]

ROLES_GENERIQUES_A_IGNORER = [
    "arrivants", "everyone", "@everyone", "candidat", "candidats", "spectateur",
    "spectateurs", "orga", "orgas", "admin", "administrateur", "bot", "booster"
]
...
# =======================================================
# 3. RÉSUMÉ DU SOIR & QUESTIONS CONFESSIONNAL
# =======================================================
...
# =======================================================
# 5. ASSISTANCE RECHERCHE & AUDIT FACTUEL
# =======================================================
...
async def verifier_echanges_recherche_ia(joueur_mystere: str, transcript: str) -> str:
    prompt = (
        ...
        "4. En cas d'erreur ou d'ambiguïté, fournis la correction exacte avec les FAITS et SOURCES précises.\n\n"
        ...
        "📚 **Explication & Faits vérifiés (Sources) :** [Détails précis]"
    )
...
async def decoder_charabia_ia(nom_membre: str, texte_brut: str) -> str:
    ...
    try:
        ...
        return f"🗣️ **Traduction d'urgence :** Le message est tellement cryptique que même les serveurs ont planté ({e})."


# ==========================================
# 7. HORLOGE AUTOMATIQUE ET ÉVÉNEMENTS
# ==========================================
...
async def on_message(message: discord.Message):
    ...
    # 2. Détection des questions pendant l'épreuve de recherche
    ...
    # 3. Gestion de la draft interactive
...
# ==========================================
# 8. COMMANDES DUOS, CONSEILS & SALONS
# ==========================================
@bot.tree.command(
    name="creer_duos",
...
@app_commands.describe(salon="Optionnel : mentionnez le salon à vider (par défaut : salon actuel)")
@app_commands.check(est_orga_ou_admin)
...
@app_commands.describe(salon="Optionnel : mentionnez le salon à configurer (par défaut : salon actuel)")
@app_commands.check(est_orga_ou_admin)
...
async def questions_confessionnal(interaction: discord.Interaction, candidat: discord.Role = None):
    ...
    recap_messages = [
        msg.content async for msg in source_recap_channel.history(limit=4, oldest_first=False)
        if not msg.content.startswith("😴") and "JOURNAL STRATÉGIQUE" in msg.content
    ]

    if not recap_messages:
        await interaction.followup.send("⚠️ Aucun journal stratégique récent trouvé dans le salon dédié.", ephemeral=True)
        return

    dernier_recap = "\n---\n".join(reversed(recap_messages))
    consigne_cible = f"Concentre-toi UNIQUEMENT sur le joueur **{candidat_nom}**." if candidat_nom else "Choisis 3 ou 4 joueurs clés ayant des décisions stratégiques ou des votes cruciaux à gérer."
...
@bot.tree.command(name="forcer_recap_jour", description="Génère immédiatement le journal stratégique global.")
@app_commands.check(est_orga_ou_admin)
...
@bot.tree.command(name="forcer_recap_spec", description="Force immédiatement la génération du récapitulatif spectateurs.")
@app_commands.check(est_orga_ou_admin)
...
@bot.tree.command(name="resume_spectateurs", description="Génère à la demande le récapitulatif du salon spectateurs.")
@app_commands.check(est_orga_ou_admin)
...
@bot.tree.command(name="pause_taches", description="Met en pause l'envoi automatique du récap du soir.")
@app_commands.check(est_orga_ou_admin)
async def pause_taches(interaction: discord.Interaction):
    if horloge_serveur.is_running():
        horloge_serveur.stop()
    await interaction.response.send_message("⏸️ **Tâches automatiques mises en pause.**", ephemeral=True)


@bot.tree.command(name="reprendre_taches", description="Réactive l'envoi automatique du récap du soir.")
@app_commands.check(est_orga_ou_admin)
async def reprendre_taches(interaction: discord.Interaction):
    if not horloge_serveur.is_running():
        horloge_serveur.start()
    await interaction.response.send_message("▶️ **Tâches automatiques réactivées.**", ephemeral=True)
...
@bot.tree.command(name="arreter_aide_recherche", description="Désactive l'assistance IA en direct pour ce salon.")
...
@bot.tree.command(name="tirage_boules", description="Lance le tirage au sort des boules avec animation et suspense.")
@app_commands.describe(
...
@bot.tree.command(name="tirer_binomes", description="Étape 1 : Tire au sort les binômes avec animation (sans créer les salons).")
...
@bot.tree.command(name="creer_salons_binomes", description="Étape 2 : Crée les salons privés pour le dernier tirage de binômes.")
...
@bot.tree.command(name="activer_conseil", description="Isole une équipe pour le conseil : retire leurs rôles personnels.")
...
@bot.tree.command(name="desactiver_conseil", description="Restaure les rôles personnels des candidats après le conseil.")
...
@bot.tree.command(name="poser_question_flash", description="Pose une question flash avec décompte dynamique Discord.")
...
class GlobalQuizLancementView(discord.ui.View):
    ...
    async def demarrer_quiz(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not CONFIG_EPREUVE_GLOBALE["active"]:
            await interaction.response.send_message("⛔ L'épreuve est actuellement clôturée.", ephemeral=True)
            return
        ...
        for i, q in enumerate(questions, 1):
            ...
            embed_q = discord.Embed(
                ...
                description=f"# {texte_q}\n\n⏳ **Fin du chrono :** <t:{fin_ts}:R> *(`{duree}s`)*",
                color=discord.Color.gold()
            )
...
@bot.tree.command(name="configurer_epreuve", description="Étape 1 : Charge les questions de l'épreuve depuis un salon secret.")
...
@bot.tree.command(name="lancer_epreuve", description="Étape 2 : Déploie l'épreuve pour un candidat précis.")
@app_commands.describe(candidat="Candidat ciblé")
@app_commands.check(est_orga_ou_admin)
async def lancer_epreuve(interaction: discord.Interaction, candidat: discord.Member):
    await interaction.response.defer(ephemeral=True)
    if not CONFIG_EPREUVE_GLOBALE["active"]:
        await interaction.followup.send("❌ Aucune épreuve active configurée. Lance d'abord `/configurer_epreuve`.", ephemeral=True)
        return

    view = GlobalQuizLancementView(candidat=candidat)
    embed = discord.Embed(title="🏺 ÉPREUVE DE RAPIDITÉ", description=f"Bienvenue {candidat.mention} pour ton épreuve.\n👉 Clique ci-dessous dès que tu es prêt :", color=discord.Color.dark_gold())
    await interaction.channel.send(embed=embed, view=view)
    await interaction.followup.send("✅ Épreuve déployée !", ephemeral=True)


@bot.tree.command(name="pause_epreuve", description="Met en pause l'épreuve.")
@app_commands.check(est_orga_ou_admin)
async def pause_epreuve(interaction: discord.Interaction):
    ch = interaction.channel
    if ch.id in ETATS_EPREUVES_SALONS:
        ETATS_EPREUVES_SALONS[ch.id]["pause_demandee"] = True
        ETATS_EPREUVES_SALONS[ch.id]["event"].clear()
        await interaction.response.send_message("⏸️ Épreuve mise en pause après la question.", ephemeral=True)
    else:
        await interaction.response.send_message("❌ Aucune épreuve en cours.", ephemeral=True)


@bot.tree.command(name="reprendre_epreuve", description="Reprend une épreuve mise en pause.")
@app_commands.check(est_orga_ou_admin)
async def reprendre_epreuve(interaction: discord.Interaction):
    ch = interaction.channel
    if ch.id in ETATS_EPREUVES_SALONS and ETATS_EPREUVES_SALONS[ch.id]["pause_demandee"]:
        ETATS_EPREUVES_SALONS[ch.id]["pause_demandee"] = False
        ETATS_EPREUVES_SALONS[ch.id]["event"].set()
        await interaction.response.send_message("▶️ Épreuve relancée !", ephemeral=True)
    else:
        await interaction.response.send_message("❌ L'épreuve n'est pas en pause.", ephemeral=True)


@bot.tree.command(name="terminer_epreuve", description="Étape 3 : Clôture définitivement l'épreuve en cours.")
@app_commands.check(est_orga_ou_admin)
...
# ========================================================
# 10. PRÉSENTATIONS & CORRECTIONS IA
# ========================================================
async def analyser_candidat_ia(texte: str) -> dict:
    ...
    prompt = (
        ...
        f"\"\"\"{texte}\"\"\"\n\n"
        "1. Donne le prénom de la personne.\n"
...
@bot.tree.command(name="formater_presentation", description="Publie la fiche propre d'un message unique.")
@app_commands.describe(message_id_ou_lien="ID ou lien du message", salon_destination="Salon de destination")
@app_commands.check(est_orga_ou_admin)
async def formater_presentation(interaction: discord.Interaction, message_id_ou_lien: str, salon_destination: discord.TextChannel = None):
    ...
    await interaction.followup.send(f"✅ Fiche publiée dans {dest.mention} !", ephemeral=True)


@bot.tree.command(name="formater_presentation_orga", description="Publie la fiche soignée d'un orga.")
@app_commands.describe(message_id_ou_lien="ID ou lien", prenom_force="Prénom forcé", salon_destination="Salon")
@app_commands.check(est_orga_ou_admin)
async def formater_presentation_orga(interaction: discord.Interaction, message_id_ou_lien: str, prenom_force: str = None, salon_destination: discord.TextChannel = None):
    await interaction.response.defer(ephemeral=True)
    dest = salon_destination or bot.get_channel(SALON_PRESENTATION_ORGAS_ID) or interaction.channel
    msg_id = int(message_id_ou_lien.strip().split("/")[-1])
    source_msg = await interaction.channel.fetch_message(msg_id)

    img_url = source_msg.attachments[0].url if source_msg.attachments else None
    res = await analyser_candidat_ia(source_msg.content)
    p_final = prenom_force.strip().capitalize() if prenom_force else res["nom"]
    embed = creer_embed_presentation_orga(p_final, res["texte"], img_url)

    await dest.send(embed=embed)
    await interaction.followup.send(f"✅ Fiche Orga publiée dans {dest.mention} !", ephemeral=True)


@bot.tree.command(name="corriger_salon_questions", description="Corrige les fautes des questions et analyse les ambiguïtés.")
...
# ========================================================
# 11. ANNONCES & GESTION DES PRÉFIXES DE PSEUDOS
# ========================================================
@bot.tree.command(name="formater_annonce", description="Met en page l'annonce dans le salon de travail avec bloc copiable.")
...
async def formater_annonce(interaction: discord.Interaction, nombre_messages: int = 1):
    ...
    prompt = f"Aère et formate cette annonce pour Discord de façon sobre et lisible :\n\n{' '.join(msgs)}"
    ...
    await dest.send(f"📋 **Texte brut :**\n```{texte[:1800]}```")
    await interaction.followup.send(f"✅ Envoyé dans {dest.mention} !", ephemeral=True)


@bot.tree.command(name="publier_annonce", description="Publie l'annonce validée chez les candidats et l'archive.")
@app_commands.check(est_orga_ou_admin)
async def publier_annonce(interaction: discord.Interaction):
    ...
    for bloc in decouper_texte_intelligent(texte, 1900):
        await dest.send(bloc)
    if arch:
...
@bot.tree.command(name="taguer_spectateurs", description="Ajoute le préfixe [SPEC] devant les Spectateurs.")
@app_commands.check(est_orga_ou_admin)
async def taguer_spectateurs(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    role = discord.utils.get(interaction.guild.roles, name=ROLE_SPECTATEURS_NAME)
    modifies = 0
    for m in role.members:
        if not m.bot and not m.display_name.startswith("[SPEC]"):
            try:
                await m.edit(nick=f"[SPEC] {m.display_name[:25]}")
                modifies += 1
                await asyncio.sleep(0.3)
            except Exception:
                pass
    await interaction.followup.send(f"👁️ **{modifies} Spectateur(s) tagué(s).**", ephemeral=True)


@bot.tree.command(name="taguer_orgas", description="Ajoute le préfixe [ORGA] devant les Orgas.")
@app_commands.check(est_orga_ou_admin)
async def taguer_orgas(interaction: discord.Interaction):
    ...
    modifies = 0
    for m in role.members:
        if not m.bot and not m.display_name.startswith("[ORGA]") and m.top_role < interaction.guild.me.top_role:
            try:
                await m.edit(nick=f"[ORGA] {m.display_name[:25]}")
                modifies += 1
                await asyncio.sleep(0.3)
            except Exception:
                pass
    await interaction.followup.send(f"🛠️ **{modifies} Orga(s) tagué(s).**", ephemeral=True)


# ========================================================
# 12. ROAST, PRAISE & TEASING
# ========================================================
...
async def roast_cmd(interaction: discord.Interaction, cible: discord.Member, contexte: str = None):
    ...
    prompt = f"Fais un roast bienveillant et drôle sur {cible.display_name} sur Discord. Contexte: {contexte or 'Jeu amical'}."
    res = await asyncio.to_thread(gemini_client.models.generate_content, model=MODEL_NAME, contents=prompt)
...
@bot.tree.command(name="roast_hardcore", description="Analyse au laser les messages pour un clash chirurgical.")
@app_commands.describe(cible="Le membre à passer au crible", contexte="Optionnel : contexte")
@app_commands.check(est_orga_ou_admin)
async def roast_hardcore_cmd(interaction: discord.Interaction, cible: discord.Member, contexte: str = None):
    await interaction.response.defer()
    msgs = []
    for ch in interaction.guild.text_channels:
        if est_categorie_candidate(ch.category):
            async for m in ch.history(limit=40):
                if m.author.id == cible.id and m.content.strip():
                    msgs.append(f"- {m.content.strip()[:150]}")
    if not msgs:
        await interaction.followup.send(f"💀 {cible.mention} est invisible sur le serveur (zéro message récent).")
        return
    prompt = f"Fais une autopsie psychologique cynique et hilarante de {cible.display_name} basée sur ses messages :\n" + "\n".join(msgs[:25])
    res = await asyncio.to_thread(gemini_client.models.generate_content, model=MODEL_NAME, contents=prompt)
    await interaction.followup.send(f"⚡ **ROAST HARDCORE —** {cible.mention}\n\n{res.text.strip()}")


@bot.tree.command(name="teasing_annonce", description="Publie un teasing avec compte à rebours dynamique.")
@app_commands.describe(minutes="Minutes d'attente", titre_teasing="Intitulé du teasing")
@app_commands.check(est_orga_ou_admin)
async def teasing_annonce(interaction: discord.Interaction, minutes: int, titre_teasing: str = "COMMUNICATION OFFICIELLE"):
    await interaction.response.defer(ephemeral=True)
    dest = bot.get_channel(SALON_ANNONCES_CANDIDATS_ID) or interaction.channel
    ...
    embed = discord.Embed(
        ...
        description=f"🔥 **Aventuriers, tenez-vous prêts !**\n\n⏳ **Révélation :** <t:{ts}:R> *(à <t:{ts}:T>)*",
        color=discord.Color.gold()
    )
    await dest.send(embed=embed)
    await interaction.followup.send(f"✅ Teasing envoyé dans {dest.mention} !", ephemeral=True)


# ========================================================
# 13. SECRETS, COMPOSITION & EXPORTS
# ========================================================
class SecretConfessionnalView(discord.ui.View):
    ...
    async def reveler_secret(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.candidat_id and not interaction.user.guild_permissions.administrator:
            await interaction.response.send_message("⛔ Accès refusé : Ce secret est personnel.", ephemeral=True)
            return
        embed = discord.Embed(title="🗝️ TON CODE SECRET", description=f"```{self.texte_secret}```", color=discord.Color.gold())
        await interaction.response.send_message(embed=embed, ephemeral=True)


@bot.tree.command(name="deposer_secret_confessionnal", description="Dépose un secret visible uniquement par le candidat.")
@app_commands.describe(candidat="Le candidat", texte_secret="Le code secret", salon="Salon")
@app_commands.check(est_orga_ou_admin)
async def deposer_secret_confessionnal(interaction: discord.Interaction, candidat: discord.Member, texte_secret: str, salon: discord.TextChannel = None):
    await interaction.response.defer(ephemeral=True)
    target = salon or interaction.channel
    view = SecretConfessionnalView(candidat.id, texte_secret)
    embed = discord.Embed(title="🗝️ TRANSMISSION CONFIDENTIELLE", description=f"{candidat.mention}, clique ci-dessous pour voir ton secret :", color=discord.Color.dark_red())
    await target.send(content=candidat.mention, embed=embed, view=view)
    await interaction.followup.send(f"✅ Secret déposé pour {candidat.mention} !", ephemeral=True)


@bot.tree.command(name="annoncer_equipes", description="Annonce officielle de la composition des équipes.")
@app_commands.describe(role_equipe_1="Équipe 1", role_equipe_2="Équipe 2")
@app_commands.check(est_orga_ou_admin)
async def annoncer_equipes(interaction: discord.Interaction, role_equipe_1: discord.Role, role_equipe_2: discord.Role):
    await interaction.response.defer(ephemeral=True)
    dest = bot.get_channel(SALON_ANNONCES_CANDIDATS_ID) or interaction.channel

    m1 = "\n".join([f"▫️ {m.display_name} ({m.mention})" for m in role_equipe_1.members if not m.bot])
    m2 = "\n".join([f"▫️ {m.display_name} ({m.mention})" for m in role_equipe_2.members if not m.bot])

    embed = discord.Embed(title="🌴 COMPOSITION OFFICIELLE DES TRIBUS 🌴", color=discord.Color.gold())
    embed.add_field(name=f"🔴 {role_equipe_1.name.upper()}", value=m1 or "Aucun", inline=False)
    embed.add_field(name=f"🟡 {role_equipe_2.name.upper()}", value=m2 or "Aucun", inline=False)

    await dest.send(content=f"📢 {role_equipe_1.mention} {role_equipe_2.mention}", embed=embed)
    await interaction.followup.send(f"✅ Équipes annoncées dans {dest.mention} !", ephemeral=True)


@bot.tree.command(name="exporter_presentations_sheet", description="Exporte les fiches de présentations au format Google Sheets.")
@app_commands.describe(salon_source="Salon des présentations")
@app_commands.check(est_orga_ou_admin)
async def exporter_presentations_sheet(interaction: discord.Interaction, salon_source: discord.abc.GuildChannel):
    await interaction.response.defer(ephemeral=True)
    lignes = ["Candidat\tDescription"]
    async for msg in salon_source.history(limit=100):
        if msg.embeds:
            for emb in msg.embeds:
                if emb.title and ("🌴" in emb.title or "Aventurier" in str(emb.footer.text)):
                    nom = emb.title.replace("🌴", "").strip()
                    desc = emb.description.replace("\n", " ").strip() if emb.description else ""
                    lignes.append(f"{nom}\t{desc}")
    buf = io.BytesIO("\n".join(lignes).encode("utf-8"))
    await interaction.followup.send(file=discord.File(buf, "presentations.tsv"), ephemeral=True)


@bot.tree.command(name="exporter_photos_candidats", description="Télécharge toutes les photos des candidats dans un ZIP.")
@app_commands.describe(salon_source="Salon des fiches")
@app_commands.check(est_orga_ou_admin)
async def exporter_photos_candidats(interaction: discord.Interaction, salon_source: discord.abc.GuildChannel):
    await interaction.response.defer(ephemeral=True)
    photos = []
    async for msg in salon_source.history(limit=100):
        if msg.embeds:
            for emb in msg.embeds:
                if emb.image and emb.image.url:
                    nom = formater_nom_salon(emb.title.replace("🌴", ""))
                    photos.append((nom, emb.image.url))

    if not photos:
        await interaction.followup.send("❌ Aucune photo trouvée.", ephemeral=True)
        return
            ...
            for n, u in photos:
                try:
                    async with session.get(u) as r:
                        if r.status == 200:
                            z.writestr(f"{n}.jpg", await r.read())
                except Exception:
                    pass

    zip_buf.seek(0)
    await interaction.followup.send(file=discord.File(zip_buf, "photos_candidats.zip"), ephemeral=True)


# ========================================================
# 14. BILANS COMPLETS CANDIDAT & ORGANISATEUR
# ========================================================
async def generer_bilan_candidat_ia(candidat_nom: str, messages_candidat: list[str], messages_sur_candidat: list[str]) -> str:
    prompt = (
        "Tu es l'analyste en chef et showrunner d'un jeu de stratégie et d'éliminations SUR DISCORD.\n"
        f"🎯 CANDIDAT CIBLÉ : **{candidat_nom}**\n\n"
        f"CE QU'IL A DIT :\n{chr(10).join(messages_candidat[:100])}\n\n"
        f"CE QU'ON DIT SUR LUI :\n{chr(10).join(messages_sur_candidat[:100])}\n\n"
        "Rédige le bilan :\n"
        "## 🏆 1. LA NOTE DU JURY : [NOTE/10] — [TITRE]\n"
...
@bot.tree.command(name="bilan_candidat", description="Génère le bilan complet d'un candidat : Note/10, analyse et bêtisier.")
@app_commands.describe(candidat="Le candidat", limite_par_salon="Messages par salon (défaut : 80)")
@app_commands.check(est_orga_ou_admin)
async def bilan_candidat(interaction: discord.Interaction, candidat: discord.Member, limite_par_salon: int = 80):
        ...
        if est_categorie_candidate(ch.category):
            async for msg in ch.history(limit=limite_par_salon):
                if msg.author.id == candidat.id:
                    msgs_c.append(f"[#{ch.name}] {candidat.display_name}: {msg.content}")
                elif nom_clean in nettoyer_texte(msg.content):
                    msgs_s.append(f"[#{ch.name}] {msg.author.display_name}: {msg.content}")

    rapport = await generer_bilan_candidat_ia(candidat.display_name, msgs_c, msgs_s)
    dest = bot.get_channel(SALON_BILAN_CANDIDATS_ID) or interaction.channel
    ...
    await dest.send(embed=embed)
    await interaction.followup.send(f"✅ Bilan envoyé dans {dest.mention} !", ephemeral=True)


async def generer_bilan_orga_ia(orga_nom: str, messages_orga: list[str], messages_sur_orga: list[str]) -> str:
    prompt = (
        "Tu es l'auditeur impartial d'un staff Discord.\n"
        f"🎯 MEMBRE DU STAFF : **{orga_nom}**\n\n"
        f"SES ACTIONS :\n{chr(10).join(messages_orga[:100])}\n\n"
        f"REMARQUES DU SERVEUR :\n{chr(10).join(messages_sur_orga[:100])}\n\n"
        "Audit attendu :\n"
        "## 🏆 1. LA NOTE DU STAFF : [NOTE/10]\n"
        "## 📊 2. OBJECTIVITÉ, PRÉSENCE & CRÉATIVITÉ\n"
        "## 🛠️ 3. POINTS FORTS & FAILS\n"
        "## 🤡 4. LE BÊTISIER DE L'ORGA"
    )
    try:
        res = await asyncio.to_thread(gemini_client.models.generate_content, model=MODEL_NAME, contents=prompt)
        return res.text.strip()
...
@bot.tree.command(name="bilan_orga", description="Bilan complet d'un orga : Note sur 10, analyse des critères et bêtisier.")
@app_commands.describe(orga="L'orga à évaluer", limite_par_salon="Messages par salon (défaut : 80)")
@app_commands.check(est_orga_ou_admin)
    ...
    for ch in guild.text_channels:
        async for msg in ch.history(limit=limite_par_salon):
            if msg.author.id == orga.id:
                msgs_o.append(f"[#{ch.name}] {orga.display_name}: {msg.content}")
            elif nom_clean in nettoyer_texte(msg.content):
                msgs_s.append(f"[#{ch.name}] {msg.author.display_name}: {msg.content}")

    rapport = await generer_bilan_orga_ia(orga.display_name, msgs_o, msgs_s)
    dest = bot.get_channel(SALON_BILAN_ORGAS_ID) or interaction.channel
...
# ========================================================
# 15. SALONS FOCUS & ANNONCES GROUPÉES
# ========================================================
@bot.tree.command(name="creer_salon_annonces_groupe", description="Crée un salon textuel où tous les candidats sont en lecture seule.")
...
@app_commands.check(est_orga_ou_admin)
async def creer_salon_annonces_groupe(interaction: discord.Interaction, nom_salon: str, nom_categorie: str, candidat_1: discord.Member, candidat_2: discord.Member):
    await interaction.response.defer(ephemeral=True)
    guild = interaction.guild
    ...
    participants = [candidat_1, candidat_2]
    ...
    await ch.send(f"📢 **SALON D'ANNONCES**\nBienvenue {candidat_1.mention}, {candidat_2.mention} *(lecture seule)*.")
    await interaction.followup.send(f"✅ Salon créé : {ch.mention}", ephemeral=True)


@bot.tree.command(name="creer_salon_focus_candidat", description="Crée un salon où 1 candidat écrit et les autres sont en lecture seule.")
@app_commands.describe(nom_salon="Nom", nom_categorie="Catégorie", candidat_actif="Candidat qui écrit", observateur_1="Observateur")
@app_commands.check(est_orga_ou_admin)
async def creer_salon_focus_candidat(interaction: discord.Interaction, nom_salon: str, nom_categorie: str, candidat_actif: discord.Member, observateur_1: discord.Member):
    await interaction.response.defer(ephemeral=True)
    guild = interaction.guild
    cat = discord.utils.find(lambda c: nettoyer_texte(c.name) == nettoyer_texte(nom_categorie), guild.categories) or await guild.create_category(nom_categorie)
    ...
    r_obs = trouver_role_personnel(observateur_1)
    overwrites[r_obs if r_obs else observateur_1] = discord.PermissionOverwrite(view_channel=True, read_messages=True, send_messages=False, add_reactions=False)

    ch = await guild.create_text_channel(name=f"🎯・{formater_nom_salon(nom_salon)}", category=cat, overwrites=overwrites)
    await ch.send(f"🎯 **SALON FOCUS : {candidat_actif.mention}** *(écriture)* | Observateur : {observateur_1.mention} *(lecture seule)*")
    await interaction.followup.send(f"✅ Salon créé : {ch.mention}", ephemeral=True)


# ========================================================
# 16. MINUTEUR D'ÉPREUVE AVEC TEMPS ADDITIONNEL
# ========================================================
...
async def chrono_minuteur(interaction: discord.Interaction, minutes: int, epreuve: str = "Épreuve"):
    ...
    if channel.id in MINUTEURS_ACTIFS:
        await interaction.response.send_message("⚠️ Un minuteur est déjà en cours dans ce salon.", ephemeral=True)
        return
...
# ========================================================
# 17. STATISTIQUES & ANALYSES LEXICALES
# ========================================================
@bot.tree.command(name="stats_mots_candidats", description="Top des mots les plus prononcés par les candidats avec la répartition.")
...
@bot.tree.command(name="chercher_mot_candidats", description="Recherche un mot précis chez les candidats et affiche publiquement le classement.")
...
async def lancer_decodeur(interaction: discord.Interaction, cible: discord.Member):
    ...
    await interaction.channel.send(f"🌐 **DÉCODEUR UNIVERSEL ACTIVÉ SUR {cible.mention} !**")
...
async def arreter_decodeur(interaction: discord.Interaction):
    if interaction.channel.id in SESSIONS_DECODEUR:
        ...
        await interaction.channel.send("🔌 **Décodeur universel désactivé.**")
...
# ========================================================
# 19. GÉNÉRATEUR UNE DE JOURNAL (L'ÉQUIPE)
# ========================================================
async def extraire_articles_une_ia(transcriptions: str) -> dict:
    prompt = (
        "Tu es le rédacteur en chef satirique d'un grand journal sportif (type L'Équipe) couvrant une aventure Discord.\n"
        f"Voici les échanges :\n\"\"\"{transcriptions[:25000]}\"\"\"\n\n"
        "Format strict :\n"
    ...
    try:
        ...
        return {"TITRE_PRINCIPAL": "C'EST ENCORE LOUPE !", "SOUS_TITRE_PRINCIPAL": "Une journée pleine d'erreurs.", "CANDIDAT_PRINCIPAL": "Aventurier"}
...
@bot.tree.command(name="generer_une_journal", description="Génère la Une satirique du journal L'Équipe avec les moments du jour.")
@app_commands.check(est_orga_ou_admin)
async def generer_une_journal(interaction: discord.Interaction):
    ...
    await interaction.channel.send(file=discord.File(buf, "une_journal.jpg"))
    await interaction.followup.send("✅ Une publiée !", ephemeral=True)


# ==========================================
# 20. DÉMARRAGE ROBUSTE AVEC RETRY
# ==========================================
def lancer_bot_avec_retry():
    ...
    while True:
        try:
            ...
            bot.run(TOKEN)
            break
        except (discord.errors.GatewayNotFound, discord.errors.HTTPException, Exception) as e:
            tentatives += 1
            attente = min(15 * tentatives, 120)
            print(f"⚠️ Erreur passerelle/réseau ({e}). Nouvelle tentative dans {attente}s...")
            time.sleep(attente)
