...
async def creer_duels_message(interaction: discord.Interaction, message_id_ou_lien: str):
    ...
    if source_msg.embeds and source_msg.embeds[0].description:
    ...
    for num in numeros_communs:
        ...
        overwrites_text = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False, read_messages=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, read_messages=True, send_messages=True),
            cible_r: discord.PermissionOverwrite(view_channel=True, read_messages=True, read_message_history=True, send_messages=True),
        ...
        overwrites_voice = {
            guild.default_role: discord.PermissionOverwrite(view_channel=False, connect=False),
            guild.me: discord.PermissionOverwrite(view_channel=True, connect=True, speak=True, mute_members=True),
            cible_r: discord.PermissionOverwrite(view_channel=True, connect=True, speak=True, stream=True, use_voice_activation=True),
            cible_j: discord.PermissionOverwrite(view_channel=True, connect=True, speak=True, stream=True, use_voice_activation=True)
        }
        ...
        # 2. Salon Vocal
        salon_voc = await guild.create_voice_channel(
            name=f"🔊・{nom_base}",
            category=categorie,
            overwrites=overwrites_voice
        )
...
# ========================================================
# ROULETTE INTERACTIVE & REJOUABLE
# ========================================================
...
class MiseRouletteModal(discord.ui.Modal):
    def __init__(self, vue_roulette, type_pari: str):
        super().__init__(title=f"Miser sur : {type_pari.upper()}")
        self.vue_roulette = vue_roulette
        self.type_pari = type_pari

        self.mise_input = discord.ui.TextInput(
            label="Montant de votre mise (jetons)",
            placeholder="Ex: 50, 100, 250...",
            min_length=1,
            max_length=6,
            required=True
        )
        self.add_item(self.mise_input)

    async def on_submit(self, interaction: discord.Interaction):
        try:
            valeur = int(self.mise_input.value.strip())
            if valeur <= 0:
                raise ValueError
        except ValueError:
            await interaction.response.send_message("❌ Saisis un nombre entier positif valide.", ephemeral=True)
            return

        self.vue_roulette.enregistrer_pari(interaction.user, self.type_pari, valeur)
        await interaction.response.send_message(
            f"✅ Pari validé : **{valeur} jetons** sur **{self.type_pari.upper()}** !", 
            ephemeral=True
        )
        await self.vue_roulette.actualiser_panneau()


class RouletteInteractiveView(discord.ui.View):
    def __init__(self, message_origine=None):
        super().__init__(timeout=40)
        self.message = message_origine
        self.paris = {}  # {user_id: {"membre": Member, "mises": {type_pari: montant}}}
        self.phase_terminee = False

    def enregistrer_pari(self, membre: discord.Member, type_pari: str, montant: int):
        if membre.id not in self.paris:
            self.paris[membre.id] = {"membre": membre, "mises": {}}
        self.paris[membre.id]["mises"][type_pari] = self.paris[membre.id]["mises"].get(type_pari, 0) + montant

    def generer_embed_attente(self, temps_restant: int) -> discord.Embed:
        embed = discord.Embed(
            title="🎰 CASINO — TABLE DE LA ROULETTE",
            description=(
                "👉 **Cliquez sur une option pour choisir votre mise sur-mesure !**\n\n"
                f"⏳ **Tirage de la bille dans :** `{temps_restant}s`\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            ),
            color=discord.Color.gold()
        )
        ...
        if not self.paris:
            embed.add_field(name="👥 Joueurs en lice", value="*Aucune mise pour l'instant...*", inline=False)
        else:
            recap_joueurs = []
            for data in self.paris.values():
                details = ", ".join([f"{k.upper()}: {v}j" for k, v in data["mises"].items()])
                recap_joueurs.append(f"▫️ **{data['membre'].display_name}** ➔ `{details}`")
            embed.add_field(name=f"👥 Paris actifs ({len(self.paris)})", value="\n".join(recap_joueurs), inline=False)

        embed.set_footer(text="Gains : Vert x36 • Rouge/Noir/Pair/Impair x2")
        return embed

    async def actualiser_panneau(self):
        if self.message and not self.phase_terminee:
            try:
                await self.message.edit(embed=self.generer_embed_attente(temps_restant=20), view=self)
            except Exception:
                pass

    @discord.ui.button(label="🔴 Rouge (x2)", style=discord.ButtonStyle.danger, row=0)
    async def btn_rouge(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(MiseRouletteModal(self, "rouge"))

    @discord.ui.button(label="⚫ Noir (x2)", style=discord.ButtonStyle.secondary, row=0)
    async def btn_noir(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(MiseRouletteModal(self, "noir"))

    @discord.ui.button(label="🟢 Zéro (x36)", style=discord.ButtonStyle.success, row=0)
    async def btn_vert(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(MiseRouletteModal(self, "vert"))

    @discord.ui.button(label="Pair (x2)", style=discord.ButtonStyle.primary, row=1)
    async def btn_pair(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(MiseRouletteModal(self, "pair"))

    @discord.ui.button(label="Impair (x2)", style=discord.ButtonStyle.primary, row=1)
    async def btn_impair(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(MiseRouletteModal(self, "impair"))


class RejouerRouletteView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=60)

    @discord.ui.button(label="🔄 Relancer la Roulette", style=discord.ButtonStyle.green)
    async def rejouer(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(view=self)
        
        nouvelle_vue = RouletteInteractiveView()
        msg = await interaction.channel.send(embed=nouvelle_vue.generer_embed_attente(30), view=nouvelle_vue)
        nouvelle_vue.message = msg
        asyncio.create_task(lancer_cycle_roulette(msg, nouvelle_vue))

    @discord.ui.button(label="🚪 Quitter le Casino", style=discord.ButtonStyle.grey)
    async def quitter(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(view=self)
        await interaction.channel.send("🚪 **La table de roulette est désormais fermée.** Merci pour vos jeux !")


async def lancer_cycle_roulette(message: discord.Message, vue: RouletteInteractiveView):
    # Compte à rebours avant tirage
    for t in range(25, 0, -5):
        await asyncio.sleep(5)
        if vue.phase_terminee:
            return
        try:
            await message.edit(embed=vue.generer_embed_attente(temps_restant=t), view=vue)
        except Exception:
            pass

    vue.phase_terminee = True
    ...
    for user_id, data in vue.paris.items():
        joueur = data["membre"]
        profit = 0
        total_mise = sum(data["mises"].values())

        for choix, mise in data["mises"].items():
            if choix == "rouge" and couleur == "rouge":
                profit += mise * 2
            elif choix == "noir" and couleur == "noir":
                profit += mise * 2
            elif choix == "vert" and couleur == "vert":
                profit += mise * 36
            elif choix == "pair" and est_pair:
                profit += mise * 2
            elif choix == "impair" and est_impair:
                profit += mise * 2

        if profit > 0:
            net = profit - total_mise
            signe = f"+{net}" if net > 0 else str(net)
            gains_total.append(f"🏆 {joueur.mention} : **{profit} jetons** remportés *(Bilan: `{signe}j`)*")
        else:
            pertes_total.append(f"💀 {joueur.display_name} : -{total_mise} jetons")

    desc_res = (
        f"🎡 La bille s'est arrêtée sur :\n\n"
        f"# {emoji} {numero} ({couleur.upper()})\n"
        f"Propriétés : `{'PAIR' if est_pair else ('IMPAIR' if est_impair else 'ZÉRO')}`\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    )

    if gains_total:
        desc_res += "**🎉 JOUEURS GAGNANTS :**\n" + "\n".join(gains_total) + "\n\n"
    else:
        desc_res += "💀 **Aucun joueur gagnant sur ce tour ! La banque ramasse tout.**\n\n"

    if pertes_total:
        desc_res += f"*(Mises perdues : {', '.join(pertes_total)})*"

    embed_fin = discord.Embed(
        title="🎰 RÉSULTAT OFFICIEL DE LA ROULETTE",
        description=desc_res,
        color=discord.Color.green() if gains_total else discord.Color.red()
    )

    vue_rejouer = RejouerRouletteView()
    await message.channel.send(embed=embed_fin, view=vue_rejouer)


@bot.tree.command(name="roulette", description="Lance la table de Roulette interactive avec mises personnalisées.")
@app_commands.check(est_orga_ou_admin)
async def roulette(interaction: discord.Interaction):
    vue = RouletteInteractiveView()
    embed = vue.generer_embed_attente(temps_restant=30)
    await interaction.response.send_message(embed=embed, view=vue)
...
class MiseBlackjackModal(discord.ui.Modal):
    def __init__(self, vue_table):
        super().__init__(title="Rejoindre la table de Blackjack")
        self.vue_table = vue_table

        self.mise_input = discord.ui.TextInput(
            label="Votre mise pour cette main",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("❌ Saisis un nombre positif valide.", ephemeral=True)
            return

        if any(j["membre"].id == interaction.user.id for j in self.vue_table.joueurs):
            await interaction.response.send_message("⚠️ Tu es déjà assis à la table !", ephemeral=True)
            return

        if len(self.vue_table.joueurs) >= 6:
            await interaction.response.send_message("⛔ Table pleine (6 joueurs maximum).", ephemeral=True)
            return

        self.vue_table.joueurs.append({
        ...
        await interaction.response.send_message(f"✅ Installé à la table avec une mise de **{valeur} jetons** !", ephemeral=True)
        await self.vue_table.actualiser_panneau()
...
class InscriptionBlackjackView(discord.ui.View):
    def __init__(self, message=None):
        ...
        self.joueurs = []  # [{"membre": m, "mise": int, "cartes": [], "statut": str}]

    def generer_embed(self, temps: int) -> discord.Embed:
        embed = discord.Embed(
            title="♠️ TABLE DE BLACKJACK — OUVERTURE DES MISES",
            description=(
                "👉 Cliquez sur **« 🪑 Rejoindre & Miser »** pour poser vos jetons !\n\n"
                f"⏳ **Distribution dans :** `{temps}s`\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            ),
            color=discord.Color.dark_blue()
        )
        if not self.joueurs:
            embed.add_field(name="🪑 Sièges occupés (0/6)", value="*La table est vide...*", inline=False)
        else:
            lignes = [f"▫️ **{j['membre'].display_name}** : mise `{j['mise']} jetons`" for j in self.joueurs]
            embed.add_field(name=f"🪑 Sièges occupés ({len(self.joueurs)}/6)", value="\n".join(lignes), inline=False)
        return embed

    async def actualiser_panneau(self):
        if self.message:
            try:
                await self.message.edit(embed=self.generer_embed(temps=15), view=self)
            except Exception:
                pass

    @discord.ui.button(label="🪑 Rejoindre & Miser", style=discord.ButtonStyle.green)
    async def btn_rejoindre(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(MiseBlackjackModal(self))


class TourBlackjackView(discord.ui.View):
    def __init__(self, joueur_actuel: discord.Member):
...
class RejouerBlackjackView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=60)

    @discord.ui.button(label="🔄 Relancer une donne", style=discord.ButtonStyle.green)
    async def rejouer(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(view=self)

        nouvelle_vue = InscriptionBlackjackView()
        msg = await interaction.channel.send(embed=nouvelle_vue.generer_embed(20), view=nouvelle_vue)
        nouvelle_vue.message = msg
        asyncio.create_task(lancer_partie_blackjack(msg, nouvelle_vue))

    @discord.ui.button(label="🚪 Quitter la table", style=discord.ButtonStyle.grey)
    async def quitter(self, interaction: discord.Interaction, button: discord.ui.Button):
        for item in self.children:
            item.disabled = True
        await interaction.response.edit_message(view=self)
        await interaction.channel.send("🚪 **La table de Blackjack est fermée.** Merci aux joueurs !")


async def lancer_partie_blackjack(message: discord.Message, vue_inscr: InscriptionBlackjackView):
    # Compte à rebours
    for s in range(20, 0, -5):
        await asyncio.sleep(5)
        try:
            await message.edit(embed=vue_inscr.generer_embed(s), view=vue_inscr)
        except Exception:
            pass

    for item in vue_inscr.children:
        item.disabled = True
    try:
        await message.edit(view=vue_inscr)
    except Exception:
        pass

    joueurs = vue_inscr.joueurs
    if not joueurs:
        await message.channel.send("❌ Aucun joueur n'a rejoint la table. Partie annulée.")
        return

    paquet = creer_paquet()
    for j in joueurs:
        j["cartes"] = [paquet.pop(), paquet.pop()]
    carte_visible_banque = paquet.pop()
    cartes_banque = [carte_visible_banque, paquet.pop()]
    ...
    for j in joueurs:
        while True:
            score = calculer_score(j["cartes"])
            if score >= 21:
                break

            cartes_texte = " ".join(j["cartes"])
            embed_tour = discord.Embed(
                title=f"🎲 TOUR DE : {j['membre'].display_name.upper()} (Mise: {j['mise']}j)",
                description=(
                    f"🏦 **Carte visible Banque :** `{carte_visible_banque}`\n"
                    f"👤 **Vos cartes :** {cartes_texte} *(Score : **{score}**)*\n\n"
                    f"👉 {j['membre'].mention}, faites votre choix :"
                ),
                color=discord.Color.gold()
            )
            vue_tour = TourBlackjackView(joueur_actuel=j["membre"])
            msg_tour = await channel.send(embed=embed_tour, view=vue_tour)
            ...
            if vue_tour.decision == "hit":
                nouvelle_carte = paquet.pop()
                j["cartes"].append(nouvelle_carte)
                score_maj = calculer_score(j["cartes"])
                if score_maj > 21:
                    await channel.send(f"💥 **BUST !** {j['membre'].mention} tire `{nouvelle_carte}` et saute (**{score_maj}**).")
                    break
            else:
                await channel.send(f"🛑 **{j['membre'].display_name}** reste à **{score}**.")
                break

    # Tour de la Banque
    score_banque = calculer_score(cartes_banque)
    while score_banque < 17:
        cartes_banque.append(paquet.pop())
        score_banque = calculer_score(cartes_banque)

    banque_str = " ".join(cartes_banque)
    banque_bust = score_banque > 21

    # Bilan & Gains
    lignes_bilan = [
        f"🏦 **BANQUE :** {banque_str} ➔ **Total : {score_banque}** " + ("*(BUST !)*" if banque_bust else ""),
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ]

    for j in joueurs:
        score_j = calculer_score(j["cartes"])
        cartes_j = " ".join(j["cartes"])
        mise = j["mise"]

        if score_j > 21:
            bilan = f"🔴 **DÉFAITE** `(-{mise}j)`"
        elif banque_bust:
            bilan = f"🟢 **VICTOIRE !** `(+{mise}j)`"
        elif score_j > score_banque:
            bilan = f"🟢 **VICTOIRE !** `(+{mise}j)` *({score_j} vs {score_banque})*"
        elif score_j == score_banque:
            bilan = "🟡 **ÉGALITÉ (Push)** `(Mise remboursée)`"
        else:
            bilan = f"🔴 **DÉFAITE** `(-{mise}j)` *({score_j} vs {score_banque})*"

        lignes_bilan.append(f"▫️ **{j['membre'].display_name}** ({cartes_j} | Total **{score_j}**) : {bilan}")
    ...
    vue_rejouer = RejouerBlackjackView()
    await channel.send(embed=embed_resultats, view=vue_rejouer)


@bot.tree.command(name="blackjack_table", description="Ouvre une table de Blackjack multijoueur avec mises personnalisées.")
@app_commands.check(est_orga_ou_admin)
async def blackjack_table(interaction: discord.Interaction):
    vue = InscriptionBlackjackView()
    embed = vue.generer_embed(temps=25)
    await interaction.response.send_message(embed=embed, view=vue)
    msg = await interaction.original_response()
    vue.message = msg
...
GRILLES_SECOURS = [
    ...
    {
        "groupes": [
            {"lien": "Pays frontaliers de l'Allemagne", "indices": ["POLOGNE", "DANEMARK", "SUISSE", "AUTRICHE"]},
            {"lien": "Acteurs ayant incarné James Bond au cinéma", "indices": ["MOORE", "CONNERY", "CRAIG", "BROSNAN"]},
            {"lien": "Termes avec un double sens monétaire (argot)", "indices": ["BALLE", "BRIQUE", "SAC", "BATON"]},
            {"lien": "Titres d'albums cultes des Beatles", "indices": ["HELP", "REVOLVER", "ABBEY", "LET IT BE"]}
        ]
    },
    {
        "groupes": [
            {"lien": "Capitales européennes situées sur le Danube", "indices": ["VIENNE", "BRATISLAVA", "BUDAPEST", "BELGRADE"]},
            {"lien": "Prénoms féminins qui sont aussi des marques automobiles", "indices": ["MERCEDES", "ALFA", "LOTUS", "ZOE"]},
            {"lien": "Mots finissant par le son [O] sans la lettre O", "indices": ["CHAPEAU", "ASSAUT", "HERAUT", "FLAMBEAU"]},
            {"lien": "Villes ayant accueilli les Jeux Olympiques d'hiver", "indices": ["CHAMONIX", "SOCHI", "TURIN", "CALGARY"]}
        ]
    },
    {
        "groupes": [
            {"lien": "Termes de tennis pouvant désigner un vêtement ou accessoire", "indices": ["FILET", "MANCHE", "JUPE", "BANDEAU"]},
            {"lien": "Présidents américains assassinés en fonction", "indices": ["LINCOLN", "GARFIELD", "MCKINLEY", "KENNEDY"]},
            {"lien": "Satellites naturels de Jupiter", "indices": ["EUROPE", "IO", "GANYMEDE", "CALLISTO"]},
            {"lien": "Mots français prenant un X au pluriel", "indices": ["BIJOU", "CAILLOU", "CHOU", "GENOU"]}
        ]
    }
]

async def generer_mur_only_connect_ia(theme: str = "culture générale", difficulte: str = "difficile") -> dict:
    """Génère un mur Only Connect cohérent, vérifiable et sans faux liens."""
    banque_types = [
        "Cinéma & Séries (titres avec un chiffre, rôles d'un même acteur, oscars)",
        "Géographie & Villes (capitales insulaires, villes fluviales, états limitrophes)",
        "Histoire & Mythologie (batailles célèbres, souverains, traités historiques)",
        "Sport & Jeux (vainqueurs de grand chelem, clubs avec un animal sur le logo, pièces d'échecs)",
        "Langue française (mots avec une particularité orthographique réelle, expressions avec une couleur)",
        "Musique & Art (peintres célèbres, instruments à cordes, mouvements artistiques)",
        "Sciences & Nature (éléments chimiques, constellations, félins, satellites naturels)"
    ]
    types_suggeres = random.sample(banque_types, 3)
    graine = uuid.uuid4().hex[:6]

    prompt = (
        "Tu es le concepteur en chef de l'émission télévisée 'Only Connect'.\n"
        "Génère un Connecting Wall (Mur de 16 indices en 4 groupes de 4) en français.\n"
        f"Graine d'aléa : {graine} | Thème global : {theme} | Difficulté : {difficulte.upper()}\n\n"
        "RÈGLES ABSOLUES DE COHÉRENCE :\n"
        "1. Tout lien doit être FACTUEL, VRAI et VÉRIFIABLE (aucun anagramme inventé ou faux lien phonétique).\n"
        "2. Pas de catégories trop simples ('Couleurs', 'Animaux', 'Fruits').\n"
        "3. Chaque case contient 1 ou 2 mots max.\n"
        "4. Inclus des pièges croisés légitimes (des mots pouvant a priori convenir à plusieurs catégories).\n\n"
        f"Domaines inspirants : {', '.join(types_suggeres)}.\n\n"
        "FORMAT STRICT (JSON BRUT UNIQUEMENT) :\n"
        "{\n"
        '  "groupes": [\n'
        '    {"lien": "Description indiscutable du lien 1", "indices": ["Mot1", "Mot2", "Mot3", "Mot4"]},\n'
        '    {"lien": "Description indiscutable du lien 2", "indices": ["Mot5", "Mot6", "Mot7", "Mot8"]},\n'
        '    {"lien": "Description indiscutable du lien 3", "indices": ["Mot9", "Mot10", "Mot11", "Mot12"]},\n'
        '    {"lien": "Description indiscutable du lien 4", "indices": ["Mot13", "Mot14", "Mot15", "Mot16"]}\n'
        "  ]\n"
        "}"
    )
    ...
    try:
        ...
        print(f"❌ Erreur parsing Only Connect : {e}")
...
async def arbitrer_lien_only_connect_ia(lien_attendu: str, explication_joueur: str) -> tuple[bool, str]:
    """Arbitre l'explication donnée par le joueur par rapport au lien attendu."""
    prompt = (
        "Tu es l'arbitre officiel de l'émission 'Only Connect'.\n"
        f"LIEN LOGIQUE ATTENDU : \"{lien_attendu}\"\n"
        f"EXPLICATION DONNÉE PAR LE JOUEUR : \"{explication_joueur}\"\n\n"
        "RÈGLES D'ARBITRAGE :\n"
        "1. L'explication doit cerner la particularité du lien (rejette 'des choses connues', 'des mots').\n"
        "2. Accepte si le cœur de l'idée est exact, même formulé différemment.\n\n"
        "FORMAT STRICT (OBLIGATOIRE) :\n"
        "VALIDE: <OUI ou NON>\n"
        "COMMENTAIRE: <Bref commentaire justificatif en 1 phrase>"
    )
    try:
        response = await asyncio.to_thread(
            gemini_client.models.generate_content,
...
class ExplicationLienModal(discord.ui.Modal):
    def __init__(self, vue_mur, groupe_identifie: dict):
        super().__init__(title="QUEL EST LE POINT COMMUN ?")
        ...
        self.explication_input = discord.ui.TextInput(
            ...
            placeholder="Ex: Capitales sur le Danube, acteurs de James Bond...",
            min_length=3,
            max_length=150,
            required=True
        )
        self.add_item(self.explication_input)

    async def on_submit(self, interaction: discord.Interaction):
        await interaction.response.defer()
        proposition = self.explication_input.value.strip()

        # Le groupe de 4 est valide : on le valide définitivement sur la grille (+1 pt de groupe)
        self.vue_mur.groupes_restants.remove(self.groupe_identifie)
        self.vue_mur.groupes_trouves.append(self.groupe_identifie)
        self.vue_mur.points += 1  # 1 point acquis pour avoir trouvé les 4 bons éléments
        self.vue_mur.selection_actuelle.clear()

        # Verrouillage en vert des boutons du groupe
        ...
        # Arbitrage de l'explication pour le bonus de points
        valide, avis = await arbitrer_lien_only_connect_ia(self.groupe_identifie["lien"], proposition)

        if valide:
            self.vue_mur.points += 2  # +2 points supplémentaires pour le thème
            await interaction.channel.send(
                f"🎯 **GROUPE ET THÈME PARFAITS (+3 pts au total) !**\n"
                f"🔗 **Thème validé :** {self.groupe_identifie['lien']}\n"
                f"💬 *Explication acceptée : « {proposition} »*"
            )
        else:
            await interaction.channel.send(
                f"🟡 **GROUPE CORRECT (+1 pt), MAIS THÈME REFUSÉ !**\n"
                f"Tu marques **1 point** pour avoir identifié les 4 éléments ensemble.\n"
                f"🔗 **Le vrai lien était :** **{self.groupe_identifie['lien']}**\n"
                f"*(Ton explication « {proposition} » n'était pas assez précise)*"
            )
...
class OnlyConnectBouton(discord.ui.Button):
    def __init__(self, indice: str, custom_id: str, row: int):
        super().__init__(label=indice, style=discord.ButtonStyle.secondary, custom_id=custom_id, row=row)
        self.indice = indice
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.indice in vue.selection_actuelle:
            vue.selection_actuelle.remove(self.indice)
        ...
        # 4 indices sélectionnés : vérification
        if len(vue.selection_actuelle) == 4:
            groupe_valide = None
            ...
            if groupe_valide:
                # Ouvre le modal pour tenter de trouver le thème
                await interaction.response.send_modal(ExplicationLienModal(vue, groupe_valide))
                return
            else:
                # Mauvais groupe de 4 : perte d'une vie
                vue.vies -= 1
                mauvais = list(vue.selection_actuelle)
                ...
                if vue.vies <= 0:
                    ...
                    await interaction.channel.send("💥 **Plus aucune vie ! Le mur se verrouille.**")
                    await vue.reveler_solutions()
                    return
                else:
                    await interaction.response.edit_message(embed=vue.generer_embed(), view=vue)
                    await interaction.followup.send(
                        f"❌ Ces 4 éléments ne forment aucun groupe ! Vies restantes : {'❤️' * vue.vies}",
                        ephemeral=True
                    )
                    return
...
class OnlyConnectMurView(discord.ui.View):
    def __init__(self, data_mur: dict, joueur_actif: discord.Member, difficulte: str, fin_timestamp: int):
        ...
        tous_indices = []
        for g in data_mur["groupes"]:
            tous_indices.extend(g["indices"])
        random.shuffle(tous_indices)

        for idx, mot in enumerate(tous_indices):
            self.add_item(OnlyConnectBouton(indice=mot, custom_id=f"oc_{idx}", row=idx // 4))
    ...
    def generer_embed(self) -> discord.Embed:
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"⚙️ **Difficulté :** `{badge_diff}`\n"
                f"⏳ **Temps restant :** <t:{self.fin_timestamp}:R> *(5 minutes)*\n"
                f"❤️ **Vies :** {'❤️' * self.vies if self.vies > 0 else '💀'}\n"
                f"🏆 **Score actuel :** `{self.points} / 12 points`\n\n"
                "👉 Sélectionne 4 indices formant un groupe.\n"
                "*(+1 pt pour le groupe de 4 trouvé + 2 pts bonus si le thème est deviné)*\n"
                "━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
            ),
    ...
    async def verifier_fin_de_partie(self):
        # Si 3 groupes ont été trouvés, le 4ème se résout par déduction (+1 pt pour les 4 éléments)
        if len(self.groupes_restants) == 1:
            ...
            for child in self.children:
                if isinstance(child, OnlyConnectBouton) and child.indice in dernier["indices"]:
                    child.style = discord.ButtonStyle.success
                    child.disabled = True

            await self.actualiser_affichage()
            await self.message.channel.send(
                f"🔓 **Dernier groupe résolu par déduction (+1 pt) !**\n"
                f"Indices : `{', '.join(dernier['indices'])}`\n"
                f"🔗 Thème associé : **{dernier['lien']}**"
            )
        ...
        if len(self.groupes_restants) == 0:
            ...
            # Bonus de grand chelem si le score est de 10 ou plus
            bonus = 2 if self.points >= 10 else 0
            self.points += bonus

            desc_fin = (
                f"👑 **Mur résolu par {self.joueur_actif.mention} !**\n\n"
                f"🏆 **Score final : {self.points} / 12 points** " + ("*(Bonus parfait de +2 pts inclus !)*" if bonus else "")
            )
            embed_fin = discord.Embed(title="🎉 FIN DU MUR ONLY CONNECT", description=desc_fin, color=discord.Color.gold())
    ...
    async def reveler_solutions(self):
        ...
        embed_sol = discord.Embed(
            ...
            description="Voici l'ensemble des connexions qu'il fallait déduire :\n\n" + "\n".join(solutions) + f"\n\nScore final : **{self.points} points**.",
            color=discord.Color.red()
        )
...
@bot.tree.command(
    name="only_connect",
    description="Lance un mur Only Connect (16 indices, 4 groupes, 5 vies, 5 minutes de jeu)."
)
@app_commands.describe(
    joueur="Le candidat au pupitre",
    difficulte="Complexité des connexions",
    theme="Thème global (ex: Culture générale, Sport, Cinéma, Géographie...)"
)
@app_commands.choices(difficulte=[
    app_commands.Choice(name="🟡 Moyen (Accessible, pièges légers)", value="moyen"),
    app_commands.Choice(name="🔴 Difficile (Style émission, pièges croisés)", value="difficile"),
    app_commands.Choice(name="🟣 Diabolique (Liens obscurs, très pointu)", value="diabolique")
])
@app_commands.check(est_orga_ou_admin)
...
# ========================================================
# MODULE : DÉPOUILLEMENT AU TEMPO KOH-LANTA (FICHIERS JPEG)
# ========================================================
...
# Chemins des fichiers locaux déposés sur le serveur / GitHub
CHEMIN_PARCHEMIN_JPEG = "parchemin_vierge.jpeg"
CHEMIN_BRASIER_JPEG = "brasier.jpeg"

# Mémoire des sessions actives : { channel_id: dict }
SESSIONS_CONSEIL = {}


# --------------------------------------------------------
# 1. VISUELS & EFFETS (PILLOW)
# --------------------------------------------------------

def charger_police(taille: int):
    """Charge une police Serif/Bold système ou la police par défaut."""
    chemins_possibles = [
...
async def generer_orthographe_bancale_ia(nom_original: str) -> str:
    ...
    prompt = (
        ...
        "RÈGLE STRICTE : Le nom DOIT rester immédiatement compréhensible et reconnaissable.\n"
        "Renvoie UNIQUEMENT le prénom déformé en MAJUSCULES, sans aucun autre texte ni guillemets."
    )
    try:
        response = await asyncio.to_thread(
            gemini_client.models.generate_content,
        ...
        texte = response.text.strip().replace('"', '').upper()
        return texte if len(texte) < 25 else nom_original.upper()
    except Exception:
        variantes = {
            nom_original.upper(): [
                nom_original.upper() + "E",
                nom_original.upper().replace("C", "K") if "C" in nom_original.upper() else nom_original.upper() + "H"
            ]
        }
...
def creer_image_parchemin_depuis_jpeg(nom_affiche: str) -> io.BytesIO:
    ...
    if os.path.exists(CHEMIN_PARCHEMIN_JPEG):
        ...
        # Secours neutre si le fichier est manquant
        img = Image.new("RGBA", (800, 800), (228, 205, 160, 255))

    largeur, hauteur = img.size
    draw = ImageDraw.Draw(img)

    # Taille adaptée au parchemin
    ...
    # Décalage légèrement sous le rouleau supérieur
...
def generer_image_candidat_en_flammes_depuis_jpeg(avatar_bytes: bytes) -> io.BytesIO:
    ...
    try:
        # L'ouverture du cadre de flammes fait environ 46% de la taille de l'image
        ...
        # Masque avec bords adoucis pour fondre les contours dans les flammes
        ...
        # Positionnement dans le trou carré (centre haut)
...
@bot.tree.command(
    ...
    description="Initialise une séance de dépouillement dans ce salon."
)
@app_commands.describe(total_votants="Nombre total de bulletins déposés dans l'urne")
@app_commands.check(est_orga_ou_admin)
async def demarrer_conseil(interaction: discord.Interaction, total_votants: int):
    if total_votants < 2:
        await interaction.response.send_message("❌ Le nombre de votants doit être d'au moins 2.", ephemeral=True)
        return
    ...
    embed = discord.Embed(
        ...
        description=(
            f"📥 **{total_votants} aventuriers** ont voté ce soir.\n"
            f"⚖️ Seuil d'élimination : **{seuil_majorite} voix** pour sceller un destin.\n\n"
            "*« Je vais procéder au dépouillement des votes. Si quelqu'un possède un collier d'immunité et souhaite le jouer, c'est le moment de me le remettre. »*"
        ),
        color=discord.Color.dark_orange()
    )
    embed.set_footer(text="Tapez /depouiller_vote pour sortir le premier bulletin")
    await interaction.response.send_message(embed=embed)


@bot.tree.command(
    name="depouiller_vote",
    description="Sors et affiche le prochain bulletin de l'urne."
)
@app_commands.describe(candidat="La personne dont le nom est inscrit sur ce bulletin")
@app_commands.check(est_orga_ou_admin)
async def depouiller_vote(interaction: discord.Interaction, candidat: discord.Member):
    ...
    # 1. Visuel du bulletin depuis parchemin_vierge.jpeg
    nom_bancal = await generer_orthographe_bancale_ia(candidat.display_name)
    img_buf = await asyncio.to_thread(creer_image_parchemin_depuis_jpeg, nom_bancal)
    ...
    for uid, count in session["decompte"].most_common():
        ...
        nom = mb.display_name if mb else f"ID {uid}"
        lignes_scores.append(f"▫️ **{nom}** : {count} vote{'s' if count > 1 else ''}")

    embed_vote = discord.Embed(
        title=f"📜 BULLETIN #{index} SUR {session['total_votants']}",
        description=(
            f"Le nom inscrit est : **{nom_bancal}** *(pour {candidat.mention})*.\n\n"
            f"📊 **État de l'urne :**\n" + "\n".join(lignes_scores)
        ),
    ...
    if votes_restants > 0:
        ...
        if candidats_danger:
            mentions_danger = ", ".join([m.mention for m in candidats_danger if m])
            embed_suspense = discord.Embed(
                title="⚡ SUSPENSE : LE PROCHAIN VOTE PEUT ÊTRE DÉCISIF !",
                description=f"Si le nom de {mentions_danger} ressort au prochain bulletin, son élimination sera définitive !",
                color=discord.Color.gold()
            )
            await interaction.channel.send(embed=embed_suspense)

    # 5. Dépouillement terminé sans majorité absolue
